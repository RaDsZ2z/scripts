...
"""
Nano Banana (Gemini) 图片生成 - Python版本 (ikuncode)
...
class GeminiImageGenerator:
    def __init__(self, api_key: str, api_url: str = "https://api.ikuncode.cc/v1/chat/completions"):
        """
        初始化Gemini图片生成器
...
def main():
    # ===== 固定配置，一般不需要修改 =====
    api_key = get_api_key("IKUN_BANANA_API_KEY")
    if not api_key:
        raise SystemExit("IKUN_BANANA_API_KEY is not configured in api_keys.env")
    OUTPUT_DIR = "output"
    # ====================================
    ...
    print("=" * 60)
    print("  Nano Banana (Gemini) 图片生成器 (ikuncode)")
    print("=" * 60)
